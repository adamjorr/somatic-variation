...
samtools sort -@ 24 -m 2G -O bam -o e_mel_realigned_sorted_1.bam e_mel_realigned_1.bam #sort output before variant calling
bash create_consensus.sh -o e_mel_1.fa -r ../e_grandis_f11_cp.fa -i e_mel_realigned_sorted_1.bam #make a new consensus reference from the alignment
cd ..
...
samtools sort -@ 24 -m 2G -O bam -o e_mel_realigned_sorted_2.bam e_mel_realigned_2.bam #sort output before variant calling
bash create_consensus.sh -o e_mel_2.fa -r ../e_mel_1/e_mel_1.fa -i e_mel_realigned_sorted_2.bam #make a new consensus reference from the alignment
cd ..
...
samtools sort -@ 24 -m 2G -O bam -o e_mel_realigned_sorted_3.bam e_mel_realigned_3.bam #sort output before variant calling
bash create_consensus.sh -o e_mel_3.fa -r ../e_mel_2/e_mel_2.fa -i e_mel_realigned_sorted_3.bam #make a new consensus reference from the alignment
cd ..

#call variants
bash bwa_aligner.sh -r e_mel_3/e_mel_3.fa -o alignment.bam -i $READ_DIR #align the raw reads to our new pseudo-reference
samtools sort -@ 24 -m 2G -O bam -o alignment_sorted.bam alignment.bam #sort output before variant calling
bash gatkcaller.sh -o var-calls.vcf -r e_mel_3/e_mel_3.fa -i alignment_sorted.bam #use GATK to call variants. we use unfiltered reads so we don't mess up assumptions made by GATK.
#outputs var-calls.vcf, the variant calls

#filter variants and make tree
bash vcf2tree.sh -t 12 -g 3 -i var-calls.vcf -o tree.nwk #the fasta alignment of the SNPs is called cleaned.fasta
Rscript plot_tree.R tree.nwk tree.pdf
